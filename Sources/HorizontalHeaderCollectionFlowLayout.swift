...
open class HorizontalHeaderCollectionFlowLayout: UICollectionViewFlowLayout {
    ...
    override open func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        attributesArray?.forEach({ if $0.representedElementKind == UICollectionView.elementKindSectionHeader { missingSections.remove(($0.indexPath as NSIndexPath).section) } })
        ...
        attributesArray?.forEach({ (attributes: UICollectionViewLayoutAttributes) in
            if attributes.representedElementKind != UICollectionView.elementKindSectionHeader { return }
            ...
            if items > 0 {
                ...
                firstAttributes = layoutAttributesForSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, at: IndexPath(item: 0, section: section))
                lastAttributes = layoutAttributesForSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, at: IndexPath(item: max(items - 1, 0), section: section))
            }
            let topHeaderWidth = items > 0 ? attributes.frame.width : 0
            let bottomHeaderWidth = attributes.frame.width
            var frame = attributes.frame.inset(by: collectionView.contentInset)
            frame = frame.inset(by: sectionInset)
